...
| **parry3d-f64** 0.30.2 *(installed 2026-08-22, M3 T6)* | Collision detection — bar-on-bar segment distance (the `check_bar_collisions` primitive) | [03](./03-tech-stack.md), [§D](./08-architecture-spec.md#d--wasm--typescript-boundary), M3 plan Q2 |
| **nalgebra** | Linear algebra (vectors, matrices, transforms) for geometry math — **not needed so far** (parry3d-f64 0.30 pulls glamx/glam, not nalgebra) | [03](./03-tech-stack.md) |
...
**M3 T6 parry3d verdict (2026-08-22 — the Q2 decision gate PASSED on all three criteria; parry3d-f64 ADOPTED, the locked-stack choice confirmed):** the spike probed parry3d-f64 0.30.2 (`default-features = false` + `std`/`dim3`/`f64`) against an analytic segment–segment reference (Ericson's closest-point parameterization). **(i) Precision ✅** — agreement within the 1e-6 mm criterion over the whole corpus: 8 structured cases (parallel / crossing / skew / endpoint-clamped / collinear / near-parallel / building-scale coordinates) + 500 randomized skew segment pairs + 120 randomized bent-bar polylines (deterministic fixed-seed LCG, no rand crate); the corpus agreement test stays as a permanent regression pin (`core/src/collision/tests.rs`, the analytic reference kept as the documented pure-math fallback that was not needed). **(ii) WASM footprint ✅** — core.wasm raw: 38.2 kB (T2 baseline) → 46.0 kB (+7.8 kB: the clash engine + `ClashPairs` boundary struct) → 48.7 kB with the parry probe linked (+2.7 kB: LTO + wasm-opt strip everything but the segment-distance path) → **46.3 kB final shipped** (parry adopted, spike export removed; 19.3 kB gzip). **(iii) 1,000-bar shape ✅** — all-pairs-with-prefilter over a 1,002-bar corpus: ~4.4 ms native release (analytic and parry primitives equal), ~24 ms through the full WASM/TS boundary incl. JS-side flattening (T6 measures the shape; T7 arms the regression tripwire). **Companion tools:** `cargo audit` / `cargo deny` are not pre-installed and the `cargo install cargo-audit --locked` attempt FAILED on this windows-gnu host (parking_lot_core build-script linker error — environment issue, not a crate finding); the RustSec/licence run is deferred to a CI/MSVC host (recorded in the T6 task log). Supply-chain surface reviewed manually instead: 37 crates joined the lock graph (~30 compiled), all standard permissive-licence (Apache-2.0/MIT) ecosystem crates (glamx 0.3, glam 0.33.5, simba 0.10.2, rstar 0.13.0, slab, smallvec, arrayvec, thiserror 2, …). **Variant note:** the f64 variant was probed (not plain parry3d) because the 1e-6 mm criterion at building-scale coordinates (±16 m) demands f64 — f32 gives ~1e-3 mm absolute resolution there.
