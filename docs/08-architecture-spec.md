...
> **Revised 2026-08-21 (M3 T4 — the reserved Bar Group slot (G) is activated, behavior locked → implemented; gesture model refined by author decision the same day):** click a wall face to capture it (the captured face sets the cover side — the Place Bar mechanism). Then TWO separate region actions, both committed by **Enter/Space** (never by pointer-up — the user can adjust the params against the live preview before placing): **A. whole-face** — Enter with no region drawn fills the captured face (the 90% detailing case, plan Q4-a default; edge distances stay rule params); **B. region** — drag the two corners on the face (pointer-up only DEFINES the region) or click-click them (first click = corner A, second = corner B), then Enter commits. Corner points resolve through §B.3 snapping (grid + reference endpoint/midpoint — tracing a region off a DXF background works). A live translucent preview of the generated bars tracks the drag and Properties-panel edits (cover, Ø, spacing, edge distances, orientation — catalog defaults, editable before commit). Commit dispatches ONE `placeBarGroup` command (one undo level); Esc cancels; single-shot auto-return to Select, double-click sticky (rules 1–2). Place Bar (B) stays unchanged (Q5: two tools, no mode toggle). The same face-local region + Enter-commit model is the template for slab reinforcement later (author note).
