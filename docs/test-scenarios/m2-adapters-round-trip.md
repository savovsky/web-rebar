...
### M2-S12 — DXF section export: the file measures true in real CAD (the T7 acceptance probe)

**Covers:** T7 (Q5 — §A milestone acceptance sentence 3) · **Status:** ✅ manual 2026-08-18 (author — Allplan 2022 "Import AutoCAD Data", after the iteration-2 ODA ownership-graph fix) · **Headless counterpart:** `src/io/dxf-export.test.ts` (exact-coordinate text assertions + the reimport-through-our-own-importer fidelity probe), `src/commands/export-section-dxf.test.ts` (command contract: outline coords == `selectSectionPrimitives` output, circle radius == Ø/2, purity, NOT_FOUND), `src/ui/shell/dxf-status-hints.test.ts` (export hints)
