...
- **M1: ✅ COMPLETE (2026-08-09)** — Q1–Q4 answered (see below); §E revised to host-follow. M0 is ✅ complete ([tracker](./m0-one-wall-one-bar.md)); branch `A_MVP_Scope_M1`. **T1 ✅** (undo core), **T2 ✅** (edit commands + reactivity proofs), **T3 ✅** (edit UI + hover picking), **T4 ✅** (Move tool + §G.1 bounded-section revision — live-offset render), **T5 ✅** (performance probes — the §5 recompute probe passes at ~3.5 ms median; Q2-a memory confirmed ~600–1,200× under the §E estimate), **T6 ✅** (acceptance pass — the §A sentence captured in `src/commands/m1-acceptance.test.ts`, checklist verdict green on every row incl. the undo-per-command row that was N/A in M0; verdict table in the task log). **F3 RESOLVED (author decision 2026-08-09):** the `moveElement` dispatch cost at 5× reference scale (~37 ms median) is **accepted at M1 scale** — revisit with the M3/M4 edit-workflow scope (candidates in the T5/T6 task logs; the 100 ms regression tripwire in `m1-performance.test.ts` stays armed).
...
### T6 — M1 acceptance pass: `m1-acceptance.test.ts` + checklist audit + docs/scenarios sweep ✅ (2026-08-09, approved by the author — commit hash in the tracker row)

Not a feature task — the §A milestone acceptance sentence captured as a durable headless test, a rule-by-rule audit of T1–T5 against the root README "Rules for Implementation Sessions" + Review Checklist (**including the "Undo/redo (§E) works for every newly added command" row that was N/A in M0**), and the docs/scenarios sweep. **Zero app-behavior impact:** one new test file + docs (mirrors M0 T11).

**Durable artifact:** `src/commands/m1-acceptance.test.ts` (7 tests; the reference-scale part reuses the T5 `reference-project.ts` fixture on the T5 `createBenchmarkStore` — the 1,055-command build would cost ~44 s under the dev-only RTK invariant checks):

1. **The §A sentence end-to-end** (one wall, TWO hosted bars, one open section): `placeWall` → `placeBar` ×2 at 25 mm cover (the Place Bar tool's own face-click resolution) → `createSection` + `setActiveSection` (the open 2D view) → `moveElement` (+300 mm Z) → the wall plan-translates (thickness/height untouched) AND both bars' paths shift exactly with it (`coverDistance` kept as design intent, §C — host-follow, §E revised) → the memoized `selectSectionPrimitives` re-derives (outline follows +300; both dots follow and keep the 31 mm offset from the covered face) → **ONE undo returns the exact pre-move project reference** (the selector hands back the baseline object itself) → **redo re-applies exactly**.
2. **The same sentence at reference scale** (50 walls × 20 bars = 1,000 bars + 5 sections): one `moveElement` = **ONE undo level for the 21-action host-follow cascade** (the capped 30-level history's top snapshot IS the pre-move state — a per-action recording would have left a mid-cascade state on top); the moved wall's outline and its 20 dots shift +300 in u while the other 9 walls' content is untouched by identity; one undo restores the exact project reference, redo re-applies.
3. **30 undo levels (§E):** 37 recorded levels (2 placements + 35 moves) cap at 30; 30 undos land exactly on the oldest retained snapshot (exact reference); the 31st undo is a guarded no-op with the "Nothing to undo" hint.
4. **Every M0+M1 command undoable (registry completeness):** a probe map keyed by `CommandName` is asserted key-equal against `commandRegistry` — **a future command fails this test until its undo behavior is decided** (the Q1-a "nothing to remember" guarantee, made structural); each of the 10 project-mutating commands records exactly ONE level and restores the exact pre-command reference on undo/redo; `setActiveSection` records none (undo covers project state only, §E); `undo`/`redo` are never themselves recorded.

**Verification run:** `pnpm lint` ✅ · `pnpm test` **191/191** ✅ (184 + 7 new) · `pnpm build` ✅ (chunk-size warning is the pre-existing three.js bundle, deferred per M0 T3) · `cargo test` 19/19 ✅ (Rust untouched in M1; WASM boundary stateless — flat arrays only).

**Checklist verdict (rule → status → evidence):**

| Rule | Status | Evidence |
| --- | --- | --- |
| 1 — Command layer (§N) | ✅ | Zero `project-slice`/`undo-slice`/`schedule-slice` imports in `src/ui/` (grep); the only dispatches in `src/ui/` are registry commands + ui-slice actions (`setTool`/`setSelection`/draft lifecycle/`setCursorHint` — the §B.6-sanctioned UI-state exception, applied consistently). M1's new UI paths (Delete/Ctrl+Z/Ctrl+Shift+Z keybindings, Edit menu, Move drag commit) all dispatch commands (`deleteSelection`, `undo`, `redo`, `moveElement`) |
| 2 — Dumb components | ✅ | `Math.*` in `src/ui/` is limited to `Math.round` (cursor coordinates, canvas px sizing), `Math.PI` (arc/rotation), `Math.hypot` (pointer-travel thresholds) — interaction/display concerns; all domain math lives in `src/engine/` + React-free modules (`element-drag.ts`, `section-volume-drag.ts`, the draft modules) |
| 3 — Stateless WASM (§D) | ✅ | No `static`/`OnceCell`/`RefCell`/`Mutex`/`thread_local`/`lazy_static` in `core/src/` (grep); `cargo test` 19/19; flat slices in, flat arrays out — M1 added no Rust code |
| 4 — Data model first | ✅ | M1 added no new model types — edits operate on the M0 T2 models; the undo slice stores `ProjectState` references outside `ProjectModel` (§H.2); T2's `translateElement` grows with the element union (documented for M3/M4) |
| 5 — Doors stay open | ✅ | Undo recording is command-agnostic — any future command is auto-undoable (MCP/scripting door §N.2 WIDENED by the T6 registry-completeness probe); selection stays id-based and layer-agnostic (Layer Model door); snapshots never in `ProjectModel` (§H.2 cloud door); bar-relative moves parked for M3 with a group-highlight door note (§B.5). No Deferred Topics entry blocked |
| 6 — Design tokens only | ✅ | Zero hex/`font-size`/`px` literals in `src/ui/` outside `tokens.css` (grep); the `*_PX` constants in `constants.ts` are behavior parameters (click-drag tolerance, canvas fit padding), not styles; M1 additions (`--hover`, `--wireframe` inks, Edit menu, Move icon) are tokens-only; domain styling stays in `src/data/appearance.ts` |
| 7 — Manual test list per task | ✅ | Every M1 task report ended with one; persisted as M1-S01…S16 (all ✅ manual) + M1-S17 (this task, ⬜ pending the acceptance walkthrough) |
| 8 — Parallel edits / commit sweep | ✅ | Working tree clean at session start (HEAD `7e95e5f`); exact-match edits only; the session does not commit |
| Checklist — lint + build pass | ✅ | `pnpm lint` ✅ · `pnpm build` ✅ (see verification run) |
| Checklist — zero store mutations in `src/ui/` | ✅ | Covered under rule 1 |
| Checklist — WASM stateless / flat arrays | ✅ | Covered under rule 3 |
| **Checklist — Undo/redo (§E) works for every newly added command** | **✅** (N/A in M0) | The T6 registry probe: all 13 commands probed — 10 mutating commands record exactly ONE level with exact-reference undo/redo; `setActiveSection` records none by design; `undo`/`redo` never recorded. Plus the per-command suites (`undo.test.ts` 12, `move-element.test.ts` 6, `delete-commands.test.ts`, `delete-selection.test.ts` 7) |
| Checklist — Deferred Topics unblocked | ✅ | Covered under rule 5; the M1 plan's door check (§N.2, Layer Model, §M annotation substrate, §E/§H.2) holds after T1–T5 as built |

**F3 (escalated in T5) — RESOLVED: author ratified the T6 recommendation 2026-08-09 — (b) accept at M1 scale + (c) revisit with the M3/M4 edit-workflow scope (NOT fixed in T6 per the one-task rule):** the `moveElement` dispatch costs ~37 ms median at the 5× reference scale (20 per-bar Immer produces × O(record)). Rationale for (b)+(c): (i) the reference project is 5× the M1 acceptance scale and §L/M4 is the performance milestone; (ii) the cost is per-commit (once per completed move), not per-frame — the §5 probe proves the derived-data recompute fits the 16 ms budget; (iii) option (a) (batched single-produce cascade) trades away the explicit per-bar action-log granularity T2 chose — the log that shows every change matters for the MCP door (§N.2) — so it stays a deliberate future design change, not an acceptance-pass side effect; (iv) the clearly-labeled 100 ms regression tripwire in `m1-performance.test.ts` stays armed. **If (a) is ever picked up (M3/M4), it runs as its own task** (single `translateElements`-style reducer; T2's design note + the §E revision text updated in the same commit).

**Docs sweep (rule 7 + tracker discipline):** root README session state → T6 (M1 🟡 pending author approval; F3 deferral recorded); plans index M1 row → T6 in review; scenario file extended with **M1-S17** (the milestone acceptance walkthrough: move wall with two bars → both follow + open 2D view updates → one Ctrl+Z restores exactly → Ctrl+Shift+Z re-applies → 30+ edits then exactly 30 undos, 31st hints) + scenario index row.

**Manual test list (rule 7) — ✅ approved by the author 2026-08-09, persisted as scenario M1-S17:** full M1 regression walkthrough = **M1-S01…S16** as persisted in [../test-scenarios/m1-edit-and-reactivity.md](../test-scenarios/m1-edit-and-reactivity.md) (all previously verified — re-run as a fast regression pass), ending with the new **M1-S17 — the milestone acceptance sentence (§A):** place a wall with two bars → cut a section, keep the 2D panel open → M-drag the wall → the wall AND both bars move together in 3D, the 2D view updates on the drop (outline follows, dots keep their cover offset) → ONE Ctrl+Z restores everything exactly → Ctrl+Shift+Z re-applies → 30+ edits leave exactly 30 undo steps (the 31st shows "Nothing to undo"). Headless spot-check (optional): `pnpm test m1-acceptance` → 7 green tests (~3 s). **M1 ✅ COMPLETE (2026-08-09).**
