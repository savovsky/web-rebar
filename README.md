...
> **Next session:** **M3 T1 — PlacementGroup data model + project-slice reducers.** The M3 plan is ✅ **approved 2026-08-18** (Q1–Q8 as recommended): [m3-real-bar-placement.md](./docs/implementation-plans-and-tasks/m3-real-bar-placement.md). Implementation sessions run on branch `A_MVP_Scope_M3`; the `m3-t1-session-prompt.md` file in the repo root is the entry point. (custom writer, true 1:1 mm, `exportSectionDxf` command), branch `A_MVP_Scope_M2`.** M2 ✅ complete 2026-08-18 — T1–T8; IFC round-trip proven headless AND in the browser; DXF backgrounds import + render + snap; foreign IFC import as render-only reference solids (Q7); DXF section export author-verified in Allplan 2022; acceptance pass durable headless (`src/commands/m2-acceptance.test.ts`).
