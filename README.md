...
**M3 plan ✅ approved 2026-08-18** — [m3-real-bar-placement.md](./docs/implementation-plans-and-tasks/m3-real-bar-placement.md) (Q1–Q8 answered as recommended: parametric face-local sampling; parry3d spike-gate for collision; host-local face key + face-local region; two tools B/G; detach-on-move with regenerate-refill; `barMark` lands minimal; clashes warn non-blocking). Implementation sessions run on branch `A_MVP_Scope_M3` from the tracker's task list; the repo root carries the `m3-t<n>-session-prompt.md` file for the current task.
...
| Start implementing the current milestone (M0–M2 ✅ done — M3 active) | README → [08-architecture-spec](./docs/08-architecture-spec.md) → [03-tech-stack](./docs/03-tech-stack.md) → [M0](./docs/implementation-plans-and-tasks/m0-one-wall-one-bar.md) + [M1](./docs/implementation-plans-and-tasks/m1-edit-and-reactivity.md) trackers (patterns to reuse) |
