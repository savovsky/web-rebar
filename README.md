...
> **Last session:** 2026-08-22 — **M3 ✅ COMPLETE** on branch `A_MVP_Scope_M3` — real bar placement per §F.2: Individual (fire-and-forget, unchanged since M0) + Group (rule-based region placement with stored params; param edits regenerate; moving a member detaches it). **T1** PlacementGroup data model (Q3: host-local `faceKey` + face-local region; Q7: `barMark` + project `nextBarMark`; batch reducers) + §F.2/§H.1 dated revisions. **T2** Rust `generate_bar_group_layout` + TS orchestration (Q1 parametric face-local sampling; cover kept from ALL faces via the M0 clamp). **T3** §N `placeBarGroup` / `updatePlacementGroup` / `deletePlacementGroup` — one undo level each, registry tripwires updated same-commit. **T4** Place Bar Group tool (G — the §B.6 slot activated): face capture, region drag/click-click, live preview, Properties-panel rule params; author gesture decision: Enter/Space commits, pointer never commits. **T5** group selection/edit UX (double-click bar → group, §B.5), `moveBar` (Q6 detach-on-move), `movePlacementGroup` (author-directed Shift+grab group move → region re-target → rule-exact regenerate). **T6** collision check — parry3d-f64 ADOPTED via the Q2 decision gate (1e-6 mm corpus, +2.7 kB wasm) + non-blocking §K.4 clash reports on the placement/edit commands + the §K.1 on-demand `checkBarClashes` command (Collision Check button, author-directed). **T7** performance probes at reference scale with groups — F3 revisit RESOLVED accept-at-scale (group regenerate 20.40 ms median vs the armed 100 ms tripwire; clash check 12.52/14.69 ms; §5 recompute 2.64/6.16 ms vs 16 ms; §L.1 evidence report-only: 1,000 per-bar meshes = 1,000 draw calls → ~50,000 at the 50K-bar target — feeds M4); suite contention fixed (`vitest maxWorkers '25%'` + timeout bumps). **T8** acceptance pass — `m3-acceptance.test.ts` (5 tests: §A sentences 1–4 headless; sentence 5 = the author-run UX probe), rule-by-rule audit green (the undo-per-command row via the 25-command registry tripwire), docs/scenarios sweep — 483 vitest, shell 1,322.57 kB. M0 (One Wall, One Bar), M1 (Edit + Reactivity), M2 (Adapters Round-Trip) also ✅ complete. **M2 detail:** the adapter round-trip milestone probed both §C adapters end to end. **T1** web-ifc lazy integration + write-capability spike (Q1 gate PASSED — web-ifc writes our IFC4 files; §D.4 custom-TS-writer fallback NOT executed; Allplan 2022 imports after 3 exporter-convention fixes). **T2** IFC export adapter (Q2 intent psets, reversible compressed-GUID GlobalIds, IFC4 schema decision). **T2.5** model space Y-up → **Z-up right-handed mm** (identical to IFC/DXF; `toIfcPoint` rotation deleted — no coordinate transform at any adapter seam). **T3** IFC import + the §A round-trip identical-model probe (exact ids/geometry/intent; undo middleware now awaits async commands → ONE undo level per import); non-SPF bytes rejected before web-ifc sees them (WASM abort guard). **T4** File menu (Import/Export IFC, lazy chunks; shell 1,278 kB). **T5** DXF import core — `dxf-parser` behind our pure mapping layer (units table, block/insert explosion, bulge→arc, OCS), `ReferenceDocument` model (Q3), 3 §N commands; the 8 author fixtures probe RAN (cm/mm both directions, 67k primitives in ~0.6 s). **T6** background rendering + tracing snaps (merged linework layer, endpoint/midpoint targets per §B.3 revised; Backgrounds panel; Import DXF… + units-override submenu). **T6.5** IFC reference solids (Q7 — foreign products → render-only dummy solids via `LoadAllGeometry`; Advance Steel probe: 2,242 solids, 128k triangles, ~8 s; ONE document, ONE undo level). **T7** DXF export of the active section view (custom writer; true 1:1 mm; $INSUNITS=4; the three Q5 layers; author-verified in Allplan 2022 after the ODA/Teigha ownership-graph iteration). **T8** acceptance pass — `m2-acceptance.test.ts` (6 tests: the four §A sentences incl. the load-bearing ODA ownership-graph structure pin), rule-by-rule audit green incl. the undo-per-command row (19 commands probed), docs sweep — **359 vitest + 19 cargo green; shell 1,294.54 kB; dxf-adapter chunk 45.65 kB lazy; web-ifc chunks lazy; no INEFFECTIVE_DYNAMIC_IMPORT**. M0 (One Wall, One Bar) and M1 (Edit + Reactivity) also ✅ complete. T1 Rust/WASM `core/` crate + bridge round-trip (windows-gnu toolchain, wasm-pack 0.15); T2 data models + DIN/EC2 steel catalog seed; T3 `generate_bar_mesh` (swept cylinder, Float32/Uint32 typed arrays) + R3F smoke scene (visual confirmed); T4 RTK store (project-slice = ProjectModel, ui-slice + draft/selection state, typed hooks, Provider wired); T5 §N command layer (6 command thunks + registry + `CommandError`) + vitest (23 headless tests); T6 §B.2 app shell (top bar, tool palette with shortcuts + sticky mode, Building/Properties tabs, status bar) + doc 10 token system live (`tokens.css` → Tailwind v4 `@theme`, dark default); T7 Viewport3D (R3F canvas, §B.6 mouse mapping, token-driven grid, live status-bar coordinates) + Place Wall tool (chained click-click placement, Esc exits → `placeWall`, crosshair snap markers + translucent draft preview); T8 Place Bar tool (click a wall face → chained clicks build ONE bar with bending places — `placeBar` + `extendBar`, cover/diameter from the DIN/EC2 catalog seed, on-face grid snapping, cover kept from ALL wall faces — edges/start/end included, bends rendered with the DIN/EC2 mandrel radius per diameter (parallel-transported ring frames — no surface twist), WASM bar meshes visible through §L.2-transparent concrete); T9 section intersection + §G.1 Tier 1 sectioning orchestration (headless): Rust `plane_polyline_intersection` (0..n crossings per bar, on-plane vertices counted once) + `src/engine/sectioning.ts` — parametric wall outline at the cut plane (chord × height, not a mesh slice), cut-bar dots with true diameters (§M.4), convention-based background within viewDepth (§G.2.3), memoized `selectSectionPrimitives` selector — 83 vitest + 19 cargo tests green; T10 Section Cut tool (S — drag the line across an element, third click sets view depth/look direction, §B.6 single-shot auto-return) + resizable dockable SectionView panel (§B.2 bottom-right, opens at ¼ viewport): Canvas2D render of `selectSectionPrimitives` with a pure engine auto-fit transform (mm→px, Y-flip), dashed convention-based background, cut-bar dots at true relative diameters (§M.4), ink from design tokens + domain pen-table seed; sections show in 3D as interactive wireframe volumes (move body, stretch corner handles → `reshapeSection` command recomputes plane/depth/targets) — 127 vitest green; T11 acceptance pass against the root README review checklist — every rule verified ✅ across T1–T10 (verdict table in the M0 tracker), the §A acceptance sentence captured as a headless command-layer test (`m0-acceptance.test.ts`), findings F1/F2 report-only — **M0 ✅ COMPLETE** (128 vitest + 19 cargo green). **M1 T1 (branch `A_MVP_Scope_M1`):** undo core per §E/Q1-a/Q2-a — `undo-slice` (past/future frozen-reference snapshots, 30-level cap, session-only), RTK listener middleware recording pre-action project-state snapshots (command-scope middleware collapses cascades → one undo level per command, Q4-a), `restoreProjectSnapshot` reducer, `undo`/`redo` §N commands (registry now 10) — 140 vitest green; **M1 T2:** edit commands per §E revised host-follow — `moveElement` (`translateElement` for the wall + explicit per-bar `translateBar` cascade → one undo level restores wall+bars exactly), `deleteSection` (clears `activeSectionId` when the 2D panel showed it) — registry now 12; headless §A dependency-graph probe (`m1-reactivity.test.ts`): memoized `selectSectionPrimitives` re-derives after every edit class (move → outline follows the wall, dot keeps its 31 mm cover offset; move fully off-plane → outline/dot set empties; `deleteElement` → section drops the wall; `deleteBar` → dot gone) — 154 vitest green; **M1 T3 ✅:** edit UI entry points — `deleteSelection` command (single Delete entry point: explicit element/bar selection first, then the ACTIVE section, else status hint), Delete/Backspace + Ctrl+Z / Ctrl+Shift+Z keybindings in `use-tool-shortcuts` (same `isEditableTarget` guard; Delete inert mid-draft), Edit menu in TopBar (Radix DropdownMenu: Undo/Redo/Delete with shortcut labels + disabled states, tokens-only), undo scope middleware now joins nested command thunks into the outer scope (one undo level per composite command); review additions: hover picking (`hover-target.ts` transient store + `pickPointerWinner` shared by hover AND click — bar > wall > section volume, §B.5 revised; bars clickable through concrete AND section wireframes, `--hover` token) — 167 vitest green; scenario file `m1-edit-and-reactivity.md` started (M1-S01…S09); **M1 T4 ✅:** Move tool (M) — first §B.6 Modify-category tool (Q3-b): ToolId/shortcut/toolbar icon, live-offset drag (decided in task — the real wall + hosted bar meshes render at the transient offset, snap back on Esc; offset in a hover-target-style module store, §E), move-target picking reuses pickPointerWinner ("highlighted = what will move": wall winner → wall+bars highlight and move together; bar winner → bar alone highlights and a drag from it does NOTHING — bar-relative moves are M3 scope; §B.5 Move-hover row), grid snap on the delta (Shift disables), Esc/tool-switch cancel, click-vs-drag threshold, `commitElementDrag` → `moveElement` (host-follow, one undo level), single-shot auto-return (sticky locks); review fixes: "highlighted = what will move" picking + §G.1 revised — section content bounded by the drawn cut line segment (outlines/dots/background clipped at the line ends, matching the 3D wireframe volume; fixes the author's two-wall sideways-move probe); scenarios M1-S10…S16 persisted — 181 vitest green; **M1 T5 ✅:** performance probes (§A risks, headless) — `reference-project.ts` fixture (50 walls 5×10 grid × 20 L-bars = 1,000 bars + 5 column sections, built entirely through §N commands with the Place Bar tool's placement math; reusable by T6), `performance-probes.ts` (`createBenchmarkStore` — production middleware set; the dev-only RTK serializable/immutable invariant checks cost ~170 ms/dispatch at reference scale and are OFF in production builds; `measureRetainedBytes` — identity-deduped retained-graph walk with documented V8 estimates), `m1-performance.test.ts` (3 tests, medians over 12 runs after 3 warm-ups — regression tripwires) — **§5 full-recompute probe PASSES:** open-section recompute + 20 changed-bar meshes = 3.51 ms median (max 5.65) vs the 16 ms budget, all-5-sections bound 9.28 ms; **undo memory Q2-a CONFIRMED:** mean 8.3 KiB/level (max 14.5) vs the §E 5–10 MiB/level estimate (~600–1,200× under), whole app state + 30-level history < 1 MiB at reference scale; **F3 ESCALATED (author decision pending):** `moveElement` dispatch = 37 ms median at reference scale (20 sequential per-bar Immer produces, each O(record size)) — 100 ms regression tripwire asserted, the 16 ms budget NOT weakened; batch-cascade-into-one-produce is the candidate fix (touches T2's explicit-per-bar design) — 184 vitest green; **doc 11** (author's reinforcement authoring workflow: imported DXF/IFC backgrounds, element-by-element / floor-by-floor detailing, layer focus isolation = compute AND render scoping) added, Layer Model deferred topic cross-referenced; **M1 T6 ✅:** milestone acceptance pass — `src/commands/m1-acceptance.test.ts` (7 tests: the §A sentence end-to-end through the §N commands — place wall → two bars → cut+open section → moveElement → wall AND hosted bars update (host-follow) + the open 2D view re-derives → ONE undo restores the exact pre-move reference → redo re-applies; the same sentence at the T5 reference scale; the 30-level cap; a registry-completeness probe proving every M0+M1 command undoable — a future command fails the test until its undo behavior is decided), rule-by-rule checklist audit green on every row INCLUDING the undo-per-command row that was N/A in M0 (verdict table in the M1 tracker), scenario M1-S17 persisted; **F3 RESOLVED (author decision): (b) accepted at M1 scale + (c) revisit with the M3/M4 edit-workflow scope — the 100 ms tripwire stays armed** — **M1 ✅ COMPLETE** (191 vitest green)
> **Tooling (2026-08-08):** ESLint/Prettier stack adopted from doxeek — type-checked `typescript-eslint`, custom ruleset (max-params 2 → options objects, naming conventions, complexity limits); Prettier options live in `.prettierrc.json` (shared CLI + IDE), enforced via the `prettier/prettier` lint rule; `pnpm lint` and `pnpm build` both clean. **Testing conventions (2026-08-22, M3 T7/T8):** long-running probe/acceptance tests carry an explicit `{ timeout: 120_000 }` (the M1 T5 worker-contention rationale — a timeout bump never asserts or weakens timing semantics), and `vitest.config.ts` caps `maxWorkers: '25%'` so the suite stays green AND faster on a busy machine (unlimited forks re-transform the whole suite per worker)
> **Current phase:** **M3 ✅ COMPLETE (2026-08-22, branch `A_MVP_Scope_M3`)** — [m3-real-bar-placement.md](./docs/implementation-plans-and-tasks/m3-real-bar-placement.md) — T1–T8 all done; the five §A acceptance sentences durable (1–4 headless in `src/commands/m3-acceptance.test.ts`; 5 = the author-run UX probe, scenarios M3-T06…T26). **M4 planning next.** Previous: **M2 ✅ COMPLETE (2026-08-18, branch `A_MVP_Scope_M2`)** - [m2-adapters-round-trip.md](./docs/implementation-plans-and-tasks/m2-adapters-round-trip.md) — T1–T8 all done; the four §A acceptance sentences durable headless (`src/commands/m2-acceptance.test.ts`); IFC + DXF adapters probed end to end. **T1 ✅ complete (2026-08-18)** - web-ifc lazy integration + write-capability spike: Q1 decision gate PASSED on all three criteria (web-ifc writes our IFC4 files; §D.4 fallback NOT executed; Allplan 2022 imports the spike file completely after 3 exporter-convention fixes: wall material layer set usage, Axis shape representation, MVD-correct FILE_DESCRIPTION); **the author delivered 8 real AutoCAD DXF exports (2026-08-18) → `docs/test-fixtures/dxf/` (gitignored) - the T5/T6 hard gate is SATISFIED**. **T2 ✅ + T2.5 ✅ (2026-08-18)** - IFC export adapter (`exportIfc`); **model space migrated Y-up → Z-up right-handed (author decision)** - plan in X–Y, elevation in +Z, identical to IFC/DXF, so the adapters carry NO rotation (`toIfcPoint` deleted; §C records the convention; camera up = +Z absorbed once in the viewport). **T3 ✅ complete (2026-08-18)** - IFC import adapter (`importIfcModel`) + the §A round-trip identical-model probe GREEN (exact ids/geometry/intent, one undo level per import); undo scope middleware now awaits async commands (post-await reducers join the command's single undo level); web-ifc WASM aborts on non-SPF bytes → SPF-envelope guard before OpenModel. **T4 ✅ complete (2026-08-18)** — File menu in TopBar (Import IFC… / Export IFC; file input + blob-anchor download; status-bar hints + `ImportIfcModelSummary` skip-count summary; errors branch on `CommandError.code`; entries disabled in-flight per the serial-command-dispatch assumption; IFC stack verified in lazy chunks — shell 1,278 kB, no INEFFECTIVE_DYNAMIC_IMPORT) — **M2 Q7 scope amendment (author decision, 2026-08-18):** unsupported foreign IFC products import as render-only reference solids → new task **T6.5** (after T5/T6; trigger: the author's Advance Steel export skipped 4,008/4,008 correctly-but-uselessly). **T6.5 ✅ complete (2026-08-18, commit `737c322`)** — IFC reference solids: web-ifc LoadAllGeometry extraction (Y-up meters → Z-up model mm conventions pinned by test), render-only merged-mesh solids layer (IFC colors at reduced opacity, `--reference-solid` token fallback), ONE reference document inside the import command (one undo level); Advance Steel probe: 2,242 products → 128k triangles in ~8 s + 43 ms merge; shell 1,293 kB. T7 (DXF section export) next on branch `A_MVP_Scope_M2`. Previous: **M1 ✅ complete** (2026-08-09, branch `A_MVP_Scope_M1`) - [m1-edit-and-reactivity.md](./docs/implementation-plans-and-tasks/m1-edit-and-reactivity.md): Q1 listener-middleware undo recording, Q2 frozen-reference snapshots, Q3 dedicated Move tool (M), Q4 one undo level per command; **§E revised 2026-08-09 to host-follow** (move/copy element → hosted bars follow in the same command). M0 ✅ complete (T1-T11, 2026-08-09); **M1 ✅ complete (T1-T6, 2026-08-09 - undo/redo per §E with Q1-a listener-middleware recording + Q2-a frozen-reference snapshots, host-follow move per §E revised, Move tool, performance probes, acceptance pass; F3 resolved: accepted at M1 scale, revisit at M3/M4)**. **§A revised 2026-08-09: M2 scope widened from IFC-only to IFC + DXF adapters (author decision - find DXF tech walls early in the POC).** Task tracker: [docs/implementation-plans-and-tasks/](./docs/implementation-plans-and-tasks/README.md)
> **Next session:** **M4 planning (Multi-Element Building)** — a planning/discussion session, not an implementation task: draft the M4 plan from the recorded inputs — the M3 T7 probe table (§L.1 draw-call/CPU evidence at the 50K-bar target; the F3 revisit resolution) and the M3 T8 open items (clash-reporting for `placeBar`/`moveElement`, host-cascade for groups, cargo audit/deny on a CI/MSVC host, user-editable clash criteria — the §K.1 note). On the table: storeys/building structure, the Layer Model deferred topic (its first concrete requirement — per-group visibility inside reference documents — is recorded), slab/beam/column faces for group sampling, junction sections (§G.2.2 resolved), host reshape with groups. **M3 ✅ COMPLETE 2026-08-22** on branch `A_MVP_Scope_M3` — T1–T8; §F.2 Individual + Group placement (stored rule, edit → regenerate, Q6 detach-on-move, §E host-follow); parry3d-f64 clash engine (Q2 gate passed) with non-blocking §K.4 surfacing + the §K.1 on-demand Collision Check; acceptance pass durable headless (`src/commands/m3-acceptance.test.ts`); 483 vitest, shell 1,322.57 kB. M3 T7 ✅ complete 2026-08-22 — performance probes at reference scale with groups: **F3 revisit RESOLVED (author-approved)** — group regenerate dispatch median 20.40 ms vs the armed 100 ms tripwire (T1 batch reducers, no per-bar-produce class even with the Q8 clash check riding inside); collision check armed @ 1,000 bars (engine 12.52 ms / §K.1 on-demand command 14.69 ms medians vs 100 ms); §5 recompute re-run with group bars PASSES (2.64 ms probe / 6.16 ms bound vs 16 ms); §L.1 evidence recorded report-only (1,000 per-bar meshes = 1,000 draw calls → ~50,000 at the 50K-bar target, ~1.2–2.7 s CPU/full rebuild — feeds M4 planning); pre-existing suite contention fixed with the author (`vitest maxWorkers '25%'` + 4 timeout bumps — green AND faster); 478 vitest, gates green one pass. M2 ✅ complete 2026-08-18 — T1–T8; IFC round-trip proven headless AND in the browser; DXF backgrounds import + render + snap; foreign IFC import as render-only reference solids (Q7); DXF section export author-verified in Allplan 2022; acceptance pass durable headless (`src/commands/m2-acceptance.test.ts`).
...
| **PDF** | jsPDF 4.2 |
| **Structure** | `src/stores/` (project + ui + undo slices, typed hooks), `src/engine/` (WASM bridge + placement + group-layout/collision orchestration + sectioning/section-cut/transform math + reference-background tessellation/snapping), `src/io/` (web-ifc lazy loader, IFC export/import mapping + GUID codec, DXF import adapter + mapping — PDF stub), `src/commands/` (25 §N commands + registry), `src/ui/` (shell, toolbar, viewport, section-view, panels), `src/data/` (models + DIN/EC2 catalog seed + domain appearance) |
...
│   ├── implementation-plans-and-tasks/ ← 🔵 LIVE — approved milestone plans + task state (M3 active)
...
| --- | --- | --- | --- |
| **Layer Model** | On/off, freeze/thaw, lock/unlock, active layer per storey. User-defined layer names. **Domain input (2026-08-09):** visibility = focus isolation while detailing — hidden entities should not be re-rendered/recalculated ([doc 11](./docs/11-reinforcement-workflow.md)). **Door watch (2026-08-09):** M2's DXF background import (§A revised) needs a reference-linework storage decision that must NOT preempt this topic — **✅ resolved 2026-08-10 in the approved M2 plan Q3:** `referenceDocuments` in `ProjectModel`, deliberately NOT layers (no freeze/lock/active/storey/compute-scoping semantics; source layer names kept as inert tags; document-level render-only visibility) — full Layer Model discussion stays open for before M4 ([M2 plan](./docs/implementation-plans-and-tasks/m2-adapters-round-trip.md)). **First concrete requirement (author, 2026-08-18):** per-group visibility INSIDE reference documents ("split a file into toggleable layers") — DXF primitives already carry the inert `sourceLayer` tag for exactly this (Q3); IFC solids have no layers but natural groups (product type — plates/beams/columns — or storey containment). The minimal slice of this topic, likely its opener. | Before M4 (multi-element building) | B (interaction model) |
...
| **IFC 3D Reference Backgrounds** | Foreign IFC models (steel structures, architect BIM models — no web-rebar design intent) as render-only 3D context while detailing rebar. **✅ DECIDED 2026-08-18 (author) — M2 plan Q7 / task T6.5:** geometry-carrying foreign products import as triangulated dummy solids in `referenceDocuments` (never editable/picked/sectioned/computed; §C preserved — results stay results; intent re-derivation stays M4). Trigger: the author's Advance Steel export (4,008 products). **Follow-up requirement (author, 2026-08-18):** IFC units override in the import flow (the DXF submenu pattern) — the extraction already detects declared-vs-assumed units (`lengthUnitAssumed`); only the override param + UI are missing. An escape hatch for broken files (DXF files routinely lack units; IFC almost never — the SPF header requires them), so small and low-priority but recorded. | M2 (T6.5, after T5/T6) | C (IO adapters), Q3 referenceDocuments |
| **Reference Background Editing (cleanup & alignment)** | **Confirmed requirement (author, 2026-08-18):** clean and align imported backgrounds (DXF + IFC) against a started model — (a) whole-document transform (move/rotate/scale the reference so its orientation matches the model — real-world imports never land on our origin), (b) reference hygiene (delete/hide unnecessary items — "televisions and flowers"). Both stay inside the Q3 "reference, not model" contract: transforms and deletions of reference content, never promotion to editable model entities (promotion = the M4 foreign-intent re-derivation, §C). Technical notes from T5/T6/T6.5: content is plain model data, so commands + undo are free; the real work is PICKING (reference geometry is un-pickable by construction — needs a dedicated edit mode with per-part/per-primitive hit resolution against the merged geometries). Recorded non-goal: per-entity parametric editing of reference content. | Before M4 (with the Layer Model discussion — visibility groups and editing share the per-group/per-item indexing) | Q3 referenceDocuments, Layer Model |
...
**M3 ✅ complete 2026-08-22** — [m3-real-bar-placement.md](./docs/implementation-plans-and-tasks/m3-real-bar-placement.md) (Q1–Q8 as approved: parametric face-local sampling; parry3d-f64 adopted via the spike gate; host-local face key + face-local region; two tools B/G; detach-on-move with regenerate-refill; `barMark` minimal; clashes warn non-blocking). T1–T8 done on branch `A_MVP_Scope_M3`; no current-task prompt file remains — the next session is the M4 planning session (see Next session above).
...
| Start implementing the current milestone (M0–M3 ✅ done — M4 planning next) | README → [08-architecture-spec](./docs/08-architecture-spec.md) → [03-tech-stack](./docs/03-tech-stack.md) → the [M0](./docs/implementation-plans-and-tasks/m0-one-wall-one-bar.md) + [M1](./docs/implementation-plans-and-tasks/m1-edit-and-reactivity.md) + [M3](./docs/implementation-plans-and-tasks/m3-real-bar-placement.md) trackers (patterns to reuse) |
