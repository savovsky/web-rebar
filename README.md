...
> **Next session:** **M3 T7 — performance probes at reference scale with groups (F3 revisit; §L.1 evidence).** The `m3-t7-session-prompt.md` file in the repo root is the entry point; implementation sessions run on branch `A_MVP_Scope_M3`. M3 T6 ✅ complete 2026-08-22 — Q2 gate PASSED on all three criteria: **parry3d-f64 0.30.2 adopted** as the clash-distance primitive (1e-6 mm corpus agreement vs the analytic reference; core.wasm 38.2 → 46.3 kB raw; 1,000-bar all-pairs-with-prefilter ~4.4 ms native / ~24 ms through the boundary); `check_bar_collisions` engine (bar pairs model-wide, AABB pre-filters); Q8 non-blocking clash reports ride the four placement/move command results → status-bar warning + §K.4 danger highlight; review amendment (author direction): §K.1 on-demand `checkBarClashes` command + Collision Check top-bar button (registry 25 commands). M2 ✅ complete 2026-08-18 — T1–T8; IFC round-trip proven headless AND in the browser; DXF backgrounds import + render + snap; foreign IFC import as render-only reference solids (Q7); DXF section export author-verified in Allplan 2022; acceptance pass durable headless (`src/commands/m2-acceptance.test.ts`).
