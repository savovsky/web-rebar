...
I'm working on a browser-based reinforced concrete drawing app ("web-rebar").
Read C:\work\personal\projects\web-rebar\README.md first — it has session state
(M0 + M1 ✅ complete; M2 ✅ plan approved 2026-08-10) and the Rules for
Implementation Sessions. Then read docs/08-architecture-spec.md (especially
§A revised, §C, §D.4, §G.1, §N) and the approved plan
docs/implementation-plans-and-tasks/m2-adapters-round-trip.md (Q1–Q6 answered
as recommended — follow it; raise deviations explicitly in the task log).

Task: implement the next ⬜ Pending task from the M2 tracker (T1/T2/T2.5/T3/T4/T5/T6/T6.5 ✅
done — next is T7: DXF export of the active section view; a ready-made prompt
with the T6.5 findings lives in m2-t7-session-prompt.md) on branch A_MVP_Scope_M2,
one task at a time: pnpm lint + pnpm test + pnpm build green → changes +
manual test list → I review and commit. Do NOT commit anything yourself.

⚠️ HARD GATE (satisfied, keep verifying): the author's real DXF fixtures in
docs/test-fixtures/dxf/ and the IFC fixture in docs/test-fixtures/ifc/ are
gitignored — confirm they are still present; if missing, STOP and ask me.
```
