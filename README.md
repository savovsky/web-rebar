...
> **Next session:** **M3 planning — NOT an implementation task.** M2 has no successor plan yet; the next session discusses the M3 (Real Bar Placement) plan against the Deferred Topics table + §N before any code lands on a new branch. (custom writer, true 1:1 mm, `exportSectionDxf` command), branch `A_MVP_Scope_M2`.** M2 ✅ complete 2026-08-18 — T1–T8; IFC round-trip proven headless AND in the browser; DXF backgrounds import + render + snap; foreign IFC import as render-only reference solids (Q7); DXF section export author-verified in Allplan 2022; acceptance pass durable headless (`src/commands/m2-acceptance.test.ts`).
...
**To plan M3 (next — M2 ✅ complete), use this:**

```text
I'm working on a browser-based reinforced concrete drawing app ("web-rebar").
Read C:\work\personal\projects\web-rebar\README.md first — it has session state
(M0 + M1 + M2 ✅ complete) and the Rules for Implementation Sessions. Then read
docs/08-architecture-spec.md (especially §A milestones, §F placement, §N, and
the Deferred Topics table).

Task: discuss the M3 (Real Bar Placement) implementation PLAN — no code; the
plan gets approved before any implementation session starts. Review the M2
tracker (docs/implementation-plans-and-tasks/m2-adapters-round-trip.md) for
the door-check pattern and task granularity; probe open questions explicitly
(placement groups per §F.2; face sampling; collision detection).
```
