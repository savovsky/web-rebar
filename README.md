...
> **Next session:** **M3 T6 — collision check: parry3d decision gate + clash engine + Q8 non-blocking surfacing.** The `m3-t6-session-prompt.md` file in the repo root is the entry point; implementation sessions run on branch `A_MVP_Scope_M3`. M2 ✅ complete 2026-08-18 — T1–T8; IFC round-trip proven headless AND in the browser; DXF backgrounds import + render + snap; foreign IFC import as render-only reference solids (Q7); DXF section export author-verified in Allplan 2022; acceptance pass durable headless (`src/commands/m2-acceptance.test.ts`).
